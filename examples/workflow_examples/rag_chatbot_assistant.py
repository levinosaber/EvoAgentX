# This example shows how to use the workflow to build a RAG chatbot assistant.
# By running this file, the final workflow will be saved in the examples/output/workflow_examples_output/rag_chatbot_assistant_demo_{model_name}.json, where {model_name} is the model name used in the workflow.
...
workflow_save_path = f"examples/workflow_examples/rag_chatbot_assistant_demo_{model_name}.json"
...
async def main():
    ...
    rag_agent_query = CustomizeAgent(
        ...
        prompt_template=StringTemplate(
            instruction="You are a helpful assistant that can query the vector database. You should query the vector database by using the tool that I provide.",
        ),
        llm_config=llm_config,
        inputs=[
